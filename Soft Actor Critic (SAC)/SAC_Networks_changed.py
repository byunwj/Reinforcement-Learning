...
class SAC(object):
    def __init__(self, sess, config, state_size, action_size, action_lower_bound, action_upper_bound, alpha):
        ...
        a_loss = -tf.reduce_mean(tf.stop_gradient(min_q) - self.alpha * self.log_pi)    # to maximize the min_q - alpha*log_pi
    ...
    def train(self):
        ...
        _,_,_ =  self.sess.run([self.actor_optimizer, self.critic1_optimizer, self.critic2_optimizer],              
                          {self.states: bs, self.actions: ba, self.rewards : br, \
                           self.next_states: bs_})
